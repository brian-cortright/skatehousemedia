...
import styles from "./VideosIsland.module.css";
...
function VideosContent() {
  ...
  const { data: videoPosts, loading } = useSanityQuery(
    () => fetchVideoPosts(),
    [],
  );
  ...
  const showFilterPopup = useCallback(
    (tags: Set<string>, categories: Set<string>) => {
  ...
  return (
    <main className={styles.pageWrapper}>
      ...
      <BodyText
        ...
        margin="0 auto var(--spacing-medium_100) auto"
        textAlignment="center"
        variant="5"
      >
        {`We've pulled all of the videos from the blog posts to make things easier to watch and search through. Use the search bar to find your favorites, or filter by tags and categories.`}
      </BodyText>
      <div className={styles.searchWrapper}>
        <Button handleClick={handleFilterClick}>
          <FilterIcon />
          <BodyText color="var(--color-grey-50)" variant="5">
            Filter
          </BodyText>
        </Button>
      ...
      {filteredVideos && filteredVideos.length > 0 && (
        <div className={styles.grid}>
          {filteredVideos.map((post, index) => {
            ...
            const thumbnail = (post.thumbnail as any)?.url || "";
...
export default function VideosIsland({ pathname }: { pathname?: string }) {
  return (
